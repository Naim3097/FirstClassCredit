...
import { blogPostCards } from "./blog/cards";
...
export default async function Resources() {
  const [faqMoto, faqPhone, cmsPosts] = await Promise.all([
    getFAQs("en", "resources-motorcycle"),
    getFAQs("en", "resources-smartphone"),
    getBlogPosts("en"),
  ]);
  const motorcycleFAQItems =
    faqMoto.length > 0 ? faqsToItems(faqMoto) : vehicleFAQ;
  const smartphoneFAQItems =
    faqPhone.length > 0 ? faqsToItems(faqPhone) : smartphoneResourcesFAQ;
  const posts = cmsPosts.length > 0 ? blogPostCards(cmsPosts) : blogPosts;
