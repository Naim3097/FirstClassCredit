...
export default async function ResourcesMs() {
  const [faqMoto, faqPhone, cmsPosts] = await Promise.all([
    getFAQs("ms", "resources-motorcycle"),
    getFAQs("ms", "resources-smartphone"),
    getBlogPosts("ms"),
  ]);
  const motorcycleFAQItems =
    faqMoto.length > 0 ? faqsToItems(faqMoto) : vehicleFAQ;
  const smartphoneFAQItems =
    faqPhone.length > 0 ? faqsToItems(faqPhone) : smartphoneResourcesFAQms;
  const posts = cmsPosts.length > 0 ? blogPostCards(cmsPosts) : blogPostsMs;
    ...
    <>
      ...
      <section id="faq" className="py-20 md:py-[100px] bg-white">
        <div className="max-w-[800px] mx-auto px-5 md:px-10 lg:px-16">
          ...
          <FAQTabs
            motorcycleItems={motorcycleFAQItems}
