...
import { getEligibility, getSiteSettings, getFAQs } from "@/lib/content";
...
const FALLBACK_ELIGIBILITY = [
  ...
  { icon: "income", label: "Minimum Income", value: "RM1,500 basic monthly salary" },
  {
    ...
    label: "Documents",
    value:
      "Copy of NRIC, latest 3 months’ payslips, latest EPF statement or latest 3 months’ bank statements",
  },
];
...
export default async function FinancingHP() {
  const [elig, settings, faqs] = await Promise.all([
    getEligibility("en"),
    getSiteSettings("en"),
    getFAQs("en", "motorcycle-page"),
  ]);
  const eligGroup = elig?.motorcycle;
  const eligRows = eligGroup?.rows?.length
    ? mapEligibilityRows(eligGroup)
    : FALLBACK_ELIGIBILITY;
  const faqItems = faqs.length > 0 ? faqsToItems(faqs) : hpFAQ;
  ...
  return (
    <>
      ...
      <EligibilitySection
        ...
        pdsHref={settings?.motorcyclePdsUrl || "/motorcycle-hp-pds.pdf"}
        rows={eligRows}
      />
